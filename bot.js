...
function mensajeConfirmacion(t) {
  ...
  const botones = {
    inline_keyboard: [
      ...
      [
        { text: "✅ Listo", callback_data: `listo:${t.id}` }
      ]
    ]
  };
...
bot.on("callback_query", async (query) => {
  ...
  else if (data.startsWith("borrar:")) {
    const id = parseInt(data.split(":")[1]);
  ...
  // ── Editar campo específico ───────────────────────────────
    ...
    const etiquetas = {
      ...
      tipo:              "📊 Escribe INGRESO o EGRESO",
      entorno:           "🏠 Escribe: Personal / Negocio / Obra Majalca",
      fecha_transaccion: "📅 Nueva fecha (ej: hoy, ayer, 2024-06-03)"
    };
  ...
  // ── Cancelar ──────────────────────────────────────────────
    ...
    const registro = await obtenerRegistro(id);
    if (!registro) return;
    const { botones } = mensajeConfirmacion(registro);
...
bot.onText(/\/start/, async (msg) => {
  ...
  await bot.sendMessage(msg.chat.id,
    ...
    "Mándame un audio o texto con tus gastos e ingresos.\n" +
    "Puedes mencionar varios movimientos en un solo mensaje.\n\n" +
    "*Comandos:*\n" +
...
bot.onText(/\/ayuda/, async (msg) => {
  ...
  await bot.sendMessage(msg.chat.id,
    "*Cómo usar Fernando:*\n\n" +
    "🎙️ Manda un audio o texto con tus movimientos\n" +
    "✏️ Toca el botón Editar para corregir un registro\n" +
    "🗑️ Toca Borrar para eliminarlo\n" +
    "✅ Toca Listo para confirmar y quitar los botones\n\n" +
    "*Comandos:*\n" +
...
bot.onText(/\/ultimos/, async (msg) => {
  ...
  try {
    ...
    if (!data.length) {
      await bot.sendMessage(msg.chat.id, "📭 Sin registros aún.");
      return;
    }
